...
#   end
  ...
  test "fetches all with limit" do
    users = TestRepo.all(from u in User, order_by: [desc: u.age], limit: 2)
    names = Enum.map(users, &Map.get(&1, :name))
  ...
  test "fetches all with offset" do
    users = TestRepo.all(from u in User, order_by: [desc: u.age], offset: 1)
    names = Enum.map(users, &Map.get(&1, :name))
  ...
  test "fetches all with limit and offset" do
    users = TestRepo.all(from u in User, order_by: [desc: u.age], limit: 2, offset: 1)
    names = Enum.map(users, &Map.get(&1, :name))
  ...
  test "fetches all with pinned limit" do
    limit = 2
    users = TestRepo.all(from u in User, order_by: [desc: u.age], limit: ^limit)
    names = Enum.map(users, &Map.get(&1, :name))
  ...
  test "fetches all with pinned offset" do
    offset = 1
    users = TestRepo.all(from u in User, order_by: [desc: u.age], offset: ^offset)
    names = Enum.map(users, &Map.get(&1, :name))
  ...
  test "fetches all with pinned limit and offset" do
    limit = 2
    offset = 1
    users = TestRepo.all(from u in User, order_by: [desc: u.age], limit: ^limit, offset: ^offset)
    names = Enum.map(users, &Map.get(&1, :name))
